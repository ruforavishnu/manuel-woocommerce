...
class brand_taxonomies {
	...
	public function add_brand_fields() {
		...
		<div class="form-field term-thumbnail-wrap">
			<label><?php _e( 'Thumbnail', 'infystore' ); ?></label>
			...
			<div style="line-height: 60px;">
				...
				<button type="button" class="upload_image_button button"><?php _e( 'Upload/Add image', 'infystore' ); ?></button>
				<button type="button" class="remove_image_button button"><?php _e( 'Remove image', 'infystore' ); ?></button>
			</div>
			<script type="text/javascript">
				...
				jQuery( document ).on( 'click', '.upload_image_button', function( event ) {
					...
					file_frame = wp.media.frames.downloadable_file = wp.media({
						title: '<?php _e( "Choose an image", "infystore" ); ?>',
						button: {
							text: '<?php _e( "Use image", "infystore" ); ?>'
						},
						multiple: false
					});
	...
	public function edit_brand_fields( $term ) {
		...
		?>
		<tr class="form-field">
			<th scope="row" valign="top"><label><?php _e( 'Thumbnail', 'infystore' ); ?></label></th>
			<td>
	...
	public function product_brand_description() {
		echo wpautop( __( 'Product categories for your store can be managed here. To change the order of categories on the front-end you can drag and drop to sort them. To see more categories listed click the "screen options" link at the top of the page.', 'infystore' ) );
	}
	/**
	 * Description for shipping class page to aid users.
	 */
	public function product_attribute_description() {
		echo wpautop( __( 'Attribute terms can be assigned to products and variations.<br/><br/><b>Note</b>: Deleting a term will remove it from all products and variations to which it has been assigned. Recreating a term will not automatically assign it back to products.', 'infystore' ) );
	}
/**
	...
	public function product_brand_columns( $columns ) {
		...
		$new_columns['thumb'] = __( 'Image', 'infystore' );
	...
	public function product_brand_column( $columns, $column, $id ) {
		if ( 'thumb' == $column ) {
			...
			$columns .= '<img src="' . esc_url( $image ) . '" alt="' . esc_attr__( 'Thumbnail', 'infystore' ) . '" class="wp-post-image" height="48" width="48" />';
		}
		return $columns;
	}
	/**
